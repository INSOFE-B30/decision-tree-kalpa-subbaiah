...
```{r}
setwd("C:/insofe\\lab_assignments\\DecisionTrees_assignment")
...
trainIndex <- createDataPartition(patient_data$disease, p = .7, list = F)
...
library(DMwR)
train_data <- knnImputation(data = train_data,k=5) #KNN Imputation
sum(is.na(train_data))

test_data <- knnImputation(data = test_data,k=5) #KNN Imputation
sum(is.na(test_data))
...
trainIndex <- createDataPartition(building_data$heating_load, p = .7, list = F)

train_data <- building_data[trainIndex, ]

test_data <- building_data[-trainIndex, ]
